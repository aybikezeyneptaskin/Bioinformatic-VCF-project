...
row_linkage = linkage(jaccard_matrix, method='average')
col_linkage = linkage(jaccard_matrix.T, method='average')

# Create a clustermap with dendrograms
g = sns.clustermap(jaccard_matrix, row_linkage=row_linkage, col_linkage=col_linkage,
                   annot=True, fmt=".2f", cmap='coolwarm',
                   xticklabels=names, yticklabels=reverse_names,
                   figsize=(16, 16))

g.fig.suptitle("Jaccard Distance Heatmap with Dendrograms")
plt.show()
plt.savefig('dendogram_heatmap.png')